#uploaded by Danny, this is a reminder to reformat this code in python
...
const actions = [
    {
        id: 'flying',
        dreamMeaning: 'These dreams may be an unconscious attempt by the dreamer to escape their issues, but the act of flying suggests that they have the ability to do so, necessitating thought about fresh approaches to their issues.'
    },
    {
        id: 'floating',
        dreamMeaning: 'Dreams that float represent quick advancement, freedom, independence, peace, and tranquility. Other times, it may allude to your possessing hypnotic powers or the presence of phony pals that are jealous of you.'
    },
    {
        id: 'running',
    ...
    {
        id: 'swimming',
        dreamMeaning: 'You will probably see improvements in your life during the upcoming several months if you are calmly swimming with the current. You can be experiencing future anxiety if you\'re treading water in choppy water or swimming against the stream.'
    },
    {
        id: 'eating',
        dreamMeaning: 'Food appears in dreams frequently as something we require to sate our appetite. Even if its not usually a matter of life or death, we believe we need it on some fundamental level. Additionally, eating entails ingesting food into our bodies. Therefore, eating in a dream may represent a close relationship.'
    },
    {
        id: 'screaming',
    ...
    {
        id: 'falling',
        dreamMeaning: 'Falling in a dream typically represents losing control of a significant problem. According to psychiatrists, it also denotes fear, horror, and anxiety brought on by losing control of important things. The sensation of falling could be a representation for powerlessness.'
    },
    {
        id: 'chased',
    ...
    {
        id: 'kissing',
        dreamMeaning: 'Kissing-related dreams may be indicative of problems with personal boundaries. This dream indicates that you are having trouble setting boundaries with someone in your life or building bridges. On a platonic level, you might love and respect this individual.'
    }
]
...
app.get('/', (req,res) => {
    res.json('Welcome to Dreams API Home')
})

app.get('/news', (req, res) => {

    axios.get('https://health.clevelandclinic.org/dreams-and-dreaming/')
        .then((response ) => {
            const html = response.data
            const $ = cheerio.load(html)

            $('a:contains("dreams")', html).each(function () {
